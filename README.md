...
**Режимы работы** (`Настройки` → **Режим работы** — три карточки):

| Режим | Для чего |
|-------|----------|
| **⚡ FAST** | Максимальная скорость: Whisper small, Piper HD, **без LLM** |
| **◆ QUALITY** | Баланс: Whisper medium, LLM, ваши настройки STT |
| **🔥 HARD** | Максимум точности STT: medium+, повтор распознавания, полный LLM |

На обзоре и в шапке — бейдж текущего режима.
