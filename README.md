...
| Долго «слушает» | Нормально на CPU; режим **⚡ FAST** или слабее модель в `.env` |
...
**Режимы работы** (`Настройки` → **Режим работы**):

| Режим | Для чего |
|-------|----------|
| **⚡ FAST** | Скорость: Whisper small, Piper HD, **без LLM** |
| **◆ QUALITY** | Баланс: Whisper medium, LLM |
| **🔥 HARD** | Точность STT: medium+, повтор распознавания |

**Личность** (`Настройки` → **Личность**):

| Режим | Для чего |
|-------|----------|
| **◆ NORMAL** | Стандартный ассистент (OpenRouter) |
| **😏 SHARD SOFT** | Самоирония через OpenRouter |
| **💀 SHARD HARD** | 18+, 3 подтверждения, **без OpenRouter** — только `data/shard_hard_lines.json` и/или Ollama |

Shard Hard: скопируйте `data/shard_hard_lines.json.example` → `data/shard_hard_lines.json` (файл в `.gitignore`, заполните сами).
