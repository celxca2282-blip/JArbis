...
------------------------
  • Опиши, что делал и что ожидал.
  • Приложи файл data/jarvis.log (появится после запуска).
  • Если окно сразу пропало — приложи crash.log из папки с JArbis.exe.
  • НЕ присылай файл .env — в нём секретный ключ!


7. ВАЖНО ПРО БЕЗОПАСНОСТЬ
-------------------------
  • .env с ключом — только у тебя на ПК, никому не пересылай.
  • Это тестовая сборка, не для публичного распространения.
