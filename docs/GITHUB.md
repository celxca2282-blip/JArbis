...
2. Скачать **JArbis-v….-win64.zip** (не «Source code»)
3. Распаковать, создать `.env` из `.env.example`
4. Запустить `JArbis.exe`
5. Баги: **Issues** → **New issue** → шаблон бага
...
Для разработчиков, не обязательно для тестера:
